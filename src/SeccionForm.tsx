...
export const SeccionForm: React.FC<SeccionFormProps> = ({ initialData = {}, onSubmit, onCancel, loading }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium">Curso ID</label>
        <input className="input input-bordered w-full" value={cursoId} onChange={e => setCursoId(e.target.value)} required type="number" />
      </div>
      <div>
        <label className="block text-sm font-medium">Profesor ID</label>
        <input className="input input-bordered w-full" value={profesorId} onChange={e => setProfesorId(e.target.value)} required type="number" />
      </div>
      <div>
